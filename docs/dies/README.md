...
avant** — par e-mail, dans une vue mensuelle, et dans son agenda habituel.
...
| [`CLAUDE.md`](./CLAUDE.md) | **Les conventions de code et les cinq invariants** — à placer à la racine du dépôt `dies` sous le nom `CLAUDE.md` | Claude Code, en premier |
| [`PLAN-DEVELOPPEMENT.md`](./PLAN-DEVELOPPEMENT.md) | **Le découpage en 7 lots**, avec les consignes prêtes à copier | Le développement |
...
4. Enchaîner lot par lot. **Ne pas sauter le lot 2** (moteur de délais) ni le lot 6 (mise en service, sauvegardes).
...
## 4. Questionnaire de cadrage — les 12 questions à lui poser
...
**Ses habitudes**
8. Quel **agenda** utilise-t-elle (Outlook, Google) ? — pour le flux iCal du lot 5.
9. Sur quel **support** consultera-t-elle le plus souvent : ordinateur, téléphone, ou les deux ?
10. Peut-elle fournir son **tableur de suivi actuel** ? — il sert de jeu de test et de base à l'import du lot 3.
11. Veut-elle **stocker des documents** dans l'outil, ou seulement des liens vers son arborescence existante ?
12. Y a-t-il des **contraintes de son employeur** sur l'hébergement des données (obligation de rester sur un outil interne, validation de la DSI) ? — **à poser avant le lot 6**, c'est la seule question qui peut changer le déploiement.
...
| Documents stockés dans l'outil | Si elle préfère son arborescence existante, le lot 4 se réduit à des liens et des métadonnées |
